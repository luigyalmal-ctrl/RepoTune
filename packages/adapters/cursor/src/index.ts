...
function renderMdc(description: string, globs: string, alwaysApply: boolean, content: string): string {
...
export const cursorAdapter: AgentAdapter = {
    ...
    for (const rule of rules) {
      if (rule.scope === 'global') {
        files.push({
          ...
          strategy: 'create',
          content: renderMdc(makeDescription(rule.content), '[]', true, rule.content),
          ruleIds: [rule.id],
        });
  ...
  async validate({ repoRoot, lockFile }: AdapterValidationContext): Promise<Warning[]> {
    if (!lockFile) return [];
    const warnings: Warning[] = [];
    for (const lf of lockFile.generatedFiles.filter(f => f.agentId === 'cursor')) {
