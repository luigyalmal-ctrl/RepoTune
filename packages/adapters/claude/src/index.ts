...
export const claudeAdapter: AgentAdapter = {
	...
	async plan(rules: Rule[], repoRoot: string): Promise<AdapterPlanResult> {
		...
		if (globalRules.length > 0) {
			...
			// [SPEC CLARIFICATION] Always use managed-block for global files so lock uses
			// checksumMode managed-block from first sync and manual content outside the block is preserved.
			files.push({
				agentId: "claude",
