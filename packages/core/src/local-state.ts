...
export async function loadLocalState(repoRoot: string): Promise<LocalState> {
  try {
    return LocalStateSchema.parse(JSON.parse(await readFile(join(repoRoot, STATE), 'utf8')));
  } catch {
    return {};
  }
}

export async function saveLocalState(state: LocalState, repoRoot: string): Promise<void> {
