...
export async function loadLock(repoRoot: string): Promise<LockFile | null> {
  try {
    return LockFileSchema.parse(JSON.parse(await readFile(join(repoRoot, LOCK), 'utf8')));
  } catch {
    return null;
  }
}

export async function saveLock(lock: LockFile, repoRoot: string): Promise<void> {
