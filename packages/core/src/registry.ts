...
export async function saveRegistry(registry: Registry, repoRoot: string): Promise<void> {
  await mkdir(join(repoRoot, '.ai'), { recursive: true });
  await writeFile(join(repoRoot, REG), JSON.stringify(registry, null, 2));
}

export async function initRegistry(repoRoot: string, agents: AgentId[]): Promise<Registry> {
  const now = new Date().toISOString();
  const reg: Registry = { version: '0.1.2', createdAt: now, updatedAt: now, agents, rules: [] };
...
export async function ruleIdExists(id: string, repoRoot: string): Promise<boolean> {
...
export async function addRule(rule: Rule, repoRoot: string): Promise<void> {
  ...
  let finalId = rule.id;
  let attempt = 0;
  while (reg.rules.some(r => r.id === finalId)) {
    if (++attempt > 5) throw new Error(`Rule ID collision for '${rule.id}' after 5 attempts`);
    finalId = `${rule.id}-${attempt}`;
  }
  reg.rules.push(attempt === 0 ? rule : { ...rule, id: finalId });
  reg.rules.sort((a, b) =>
