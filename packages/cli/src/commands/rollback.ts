...
export async function runRollback(repoRoot: string, backupOpt: string | undefined, yes: boolean): Promise<void> {
  ...
  if (!backupPath) {
    ...
    backupPath = backups.length === 1
      ? backups[0]
      : await select({
          message: 'Select backup to restore:',
          choices: backups.slice(0, 10).map(b => ({ name: basename(b), value: b })),
        });
  }
  ...
  if (manifest.createdFiles.length > 0) {
    console.log(`Will delete ${manifest.createdFiles.length} file(s) (created by the sync being rolled back):`);
    for (const f of manifest.createdFiles) console.log(`  ✕ ${f}`);
  }

  if (!yes) {
    const ok = await confirm({ message: 'Restore these files?', default: false });
