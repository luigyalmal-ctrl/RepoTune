...
export async function runRuleAdd(repoRoot: string, rawContent?: string): Promise<void> {
  ...
  const scope = await select({
    ...
    choices: [
      ...
      { name: 'path — applies to specific file patterns', value: 'path' as const },
    ],
  });
  ...
  const rule: Rule = { id, content, scope, pathPattern, createdAt: now, updatedAt: now };
