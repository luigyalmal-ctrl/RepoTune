...
program.name('repotune').description('Sync AI assistant rules across agents').version('0.1.2');
...
program
  ...
  .action(async (opts: { dryRun: boolean; diff: boolean; agent: AgentId[]; yes: boolean }) => {
    const root = findRepoRoot(process.cwd());
    await runSync(root, { dryRun: opts.dryRun, diff: opts.diff, agents: opts.agent, yes: opts.yes });
  });
