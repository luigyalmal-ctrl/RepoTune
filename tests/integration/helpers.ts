...
import { addRule, createSyncEngine, initRegistry, loadRegistry, saveLock, saveLocalState } from '@repotune/core';
import type { AgentAdapter, AgentId, Rule, SyncPreview } from '@repotune/schemas';
...
export function makeRule(id: string, overrides: Partial<Rule> = {}): Rule {
  const now = new Date().toISOString();
  return { id, content: `# ${id}\nContent for ${id}.`, scope: 'global', createdAt: now, updatedAt: now, ...overrides };
}
